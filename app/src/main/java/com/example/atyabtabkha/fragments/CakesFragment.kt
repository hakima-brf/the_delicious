...
class CakesFragment : Fragment() {
    ...
    private  var cakePostAdapter: GuestPostAdapter? = null
    private var cakePostList: MutableList<Post>? = null
    ...
    override fun onCreateView(
        ...
        cakePostList = ArrayList()
        cakePostAdapter = context?.let { GuestPostAdapter(it, cakePostList as ArrayList<Post> ) }

        recyclerview.adapter = cakePostAdapter
        retreiveJusPosts()
    ...
    private fun retreiveJusPosts() {
        val userPostsRef = FirebaseDatabase.getInstance().reference.child("gests").child("cakes")
            ...
            override fun onDataChange(p0: DataSnapshot) {
                if(p0.exists()){
                    ...
                    for(snapshot in p0.children) {
                        ...
                        if (user != null) {
                            cakePostList?.add(user)
                        }


                    }
                    cakePostAdapter!!.notifyDataSetChanged()
