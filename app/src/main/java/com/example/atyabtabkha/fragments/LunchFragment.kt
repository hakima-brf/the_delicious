...
class LunchFragment : Fragment() {
    ...
    private  var lunchPostAdapter: GuestPostAdapter? = null
    private var lunchPostList: MutableList<Post>? = null
    override fun onCreateView(
        ...
        lunchPostList = ArrayList()
        lunchPostAdapter = context?.let { GuestPostAdapter(it, lunchPostList as ArrayList<Post> ) }

        recyclerview.adapter = lunchPostAdapter
        retreiveJusPosts()
    ...
    private fun retreiveJusPosts() {
        val userPostsRef = FirebaseDatabase.getInstance().reference.child("gests").child("lunch")
            ...
            override fun onDataChange(p0: DataSnapshot) {
                if(p0.exists()){
                    ...
                    for(snapshot in p0.children) {
                        ...
                        if (user != null) {
                            lunchPostList?.add(user)
                        }


                    }
                    lunchPostAdapter!!.notifyDataSetChanged()
